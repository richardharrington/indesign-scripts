...
// If the scriptArg "draftType" is set, then this was run by PlaceText.// Otherwise this script is on its own and we're assuming we want the text to be black ("Final").var myDraftType;if (!(myDraftType = app.scriptArgs.get("draftType"))) {	myDraftType = "Final";}

// If the scriptArg "storyType" is set, then this was run by PlaceText.// Otherwise it's on its own and we're assuming we want the text to be MiscellaneousBodyCopy.var myStoryType;if (!(myStoryType = app.scriptArgs.get("storyType"))) {	myStoryType = myDisplayDialog();}
...
function myDisplayDialog(){
  ...
  // First, remove hard returns from block-level html.
...
//    app.changeGrepPreferences = NothingEnum.nothing;//    app.findGrepPreferences = NothingEnum.nothing;  	if (myParagraph.characters[0].contents == "<" && myParagraph.characters[-2].contents != ">") {
  ...
  // Now shore up whitespace at the beginning and the end of block-level html.
	app.findGrepPreferences.findWhat = "^\\s*?(<.*?>)\\s*?$";
	app.changeGrepPreferences.changeTo = "$1";
	var myResults = myObject.changeGrep();
  app.changeGrepPreferences = NothingEnum.nothing;	
	//Now convert block-level html into Notes and move them to the beginning
